...
// 自动充电规则接口
interface AutoChargingRule {
  id: string;
  name: string;
  isEnabled: boolean;
  updateTime: string;
  updatedBy: string;
  chargingThreshold: number; // 充电阈值（百分比）
  selectedRobots: string[]; // 选择的机器人ID列表
}

const AutoChargingManagement: React.FC = () => {
  const { isMobile, isLargeScreen } = useResponsive();
  const [loading, setLoading] = useState(false);
  const [searchText, setSearchText] = useState('');
  const [isDrawerVisible, setIsDrawerVisible] = useState(false);
  const [editingRule, setEditingRule] = useState<AutoChargingRule | null>(null);
  ...
  // 模拟数据
  const [rules, setRules] = useState<AutoChargingRule[]>([
    {
      id: '1',
      name: '自动充电',
      isEnabled: true,
  ...
  // 处理启用/禁用
  const handleToggleEnabled = (record: AutoChargingRule) => {
    setRules(prevRules =>
      prevRules.map(rule =>
        rule.id === record.id
          ? {
  ...
  // 处理编辑
  const handleEdit = (record: AutoChargingRule) => {
    setEditingRule(record);
    form.setFieldsValue({
      name: record.name,
      isEnabled: record.isEnabled,
      chargingThreshold: record.chargingThreshold,
  ...
  // 处理删除
  const handleDelete = (record: AutoChargingRule) => {
    // 内置规则不允许删除
    if (record.id === '1') {
      message.warning('内置规则不允许删除');
      return;
    }
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (editingRule) {
            ...
            prevRules.map(rule =>
              rule.id === editingRule.id
                ? {
                    ...
                    chargingThreshold: values.chargingThreshold,
          ...
          // 新增
          const newRule: AutoChargingRule = {
            id: Date.now().toString(),
            name: values.name,
            isEnabled: values.isEnabled || false,
            updateTime: currentTime,
            updatedBy: '当前用户',
            chargingThreshold: values.chargingThreshold || 20,
  ...
  // 移动端列配置
  const mobileColumns: ColumnsType<AutoChargingRule> = [
    {
      title: '规则信息',
      key: 'ruleInfo',
      fixed: 'left',
      render: (_: any, record: AutoChargingRule) => (
        <div style={{ padding: '8px 0' }}>
          ...
          <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>
            <span>充电阈值: {record.chargingThreshold}% | 机器人: {record.id === '1' ? '全部机器人' : `${record.selectedRobots?.length || 0}台`}</span>
          </div>
          <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>
            <span>状态: </span>
            <Switch
  ...
  // 桌面端列配置
  const desktopColumns: ColumnsType<AutoChargingRule> = [
    {
      title: '名称',
      dataIndex: 'name',
      key: 'name',
      ...
      ellipsis: true,
      fixed: 'left',
      render: (text: string, record: AutoChargingRule) => (
        <Tooltip title={text}>
          <span 
    ...
    {
      title: '充电阈值',
      dataIndex: 'chargingThreshold',
      key: 'chargingThreshold',
    ...
    },
    {
      ...
      width: getColumnWidth(150),
      align: 'left',
      render: (robots: string[], record: AutoChargingRule) => {
    ...
    },
    {
      ...
      width: getColumnWidth(100),
      align: 'left',
      render: (isEnabled: boolean, record: AutoChargingRule) => (
        <Switch
          checked={isEnabled}
          onChange={() => handleToggleEnabled(record)}
        />
    ...
    },
    {
      ...
      align: 'left',
      sorter: (a: AutoChargingRule, b: AutoChargingRule) => new Date(a.updateTime).getTime() - new Date(b.updateTime).getTime(),
      render: (time: string) => {
      ...
      >
        ...
        >
          ...
          <Form.Item
            ...
            rules={[
              { required: true, message: '请输入充电阈值' },
              { type: 'number', min: 5, max: 50, message: '充电阈值范围为5%-50%' },
            ]}
            extra="单位：百分比（%），当机器人电量低于此阈值时自动充电"
          >
            <InputNumber
                placeholder="请输入百分比"
                min={5}
          ...
          <Form.Item
            ...
            extra={editingRule?.id === '1' ? '内置规则默认应用于全部机器人' : '支持多选，选择需要应用此充电规则的机器人'}
          >
            {editingRule?.id === '1' ? (
              <Select
                value={['all']}
...
export default AutoChargingManagement;