...
import { actionSequenceData, type ActionSequenceRecord, updateActionSequence } from '../../../data/actionSequenceData';
import AddActionSequence from './AddActionSequence';
import {
  Form,
  Input,
  Select,
...
// 使用共享的 ActionSequenceRecord 类型

interface ActionSequencePropertyPanelProps {
  visible: boolean;
...
const ActionSequencePropertyPanel: React.FC<ActionSequencePropertyPanelProps> = ({
  visible,
  businessProcessNode,
  onClose,
  onSave
  ...
  const [selectedProcess, setSelectedProcess] = useState<ActionSequenceRecord | null>(null);
  const [isEditDrawerVisible, setIsEditDrawerVisible] = useState(false);
  const [editingRecord, setEditingRecord] = useState<ActionSequenceRecord | null>(null);

  
  // 使用共享业务流程数据，每次渲染时获取最新数据
  const [actionSequences, setActionSequences] = useState<ActionSequenceRecord[]>([]);
  const [refreshKey, setRefreshKey] = useState(0);
  
  // 监听面板显示状态，每次打开时刷新数据
  useEffect(() => {
    if (visible) {
      const latestData = [...actionSequenceData];
      setActionSequences(latestData);
    }
  }, [visible, refreshKey]);
  ...
  useEffect(() => {
    if (visible) {
      const interval = setInterval(() => {
        const currentLength = actionSequences.length;
        const actualLength = actionSequenceData.length;
  ...
  }, [visible, actionSequences.length]);
  ...
  useEffect(() => {
    if (visible && businessProcessNode) {
      ...
      if (businessProcessNode.data?.selectedProcessId) {
        const process = actionSequences.find(p => p.id === businessProcessNode.data?.selectedProcessId);
        setSelectedProcess(process || null);
      } else {
        setSelectedProcess(null);
      }
    }
  }, [visible, businessProcessNode, form, actionSequences]);

  // 处理业务流程选择
  const handleProcessSelect = (processId: string) => {
    const process = actionSequences.find(p => p.id === processId);
    if (process) {
  ...
  return (
    ...
    >
        ...
        <Form form={form} layout="vertical">
          ...
          <Card title="业务流程选择" size="small" style={{ marginBottom: 16 }}>
            ...
            >
              ...
              >
                {actionSequences.map(process => (
                  <Option key={process.id} value={process.id}>
                    {process.sequenceName}
                  </Option>
                ))}
              </Select>
            </Form.Item>
      ...
      <AddActionSequence
        ...
        onSave={(data) => {
          if (editingRecord) {
            const updatedRecord: ActionSequenceRecord = {
              ...editingRecord,
              sequenceName: data.sequenceName || editingRecord.sequenceName,
            ...
            updateActionSequence(editingRecord.id, updatedRecord);
            
            // 更新当前选中的流程数据
            setSelectedProcess(updatedRecord);
            form.setFieldsValue({
              sequenceName: updatedRecord.sequenceName,
...
export default ActionSequencePropertyPanel;