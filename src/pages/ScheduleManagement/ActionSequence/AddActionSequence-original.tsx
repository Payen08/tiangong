...
import ActionSequencePropertyPanel from './ActionSequencePropertyPanel';
import { 
  ArrowLeftOutlined, 
  UndoOutlined, 
  RedoOutlined, 
...
interface AddActionSequenceProps {
  visible: boolean;
  onClose: () => void;
...
const AddActionSequence: React.FC<AddActionSequenceProps> = ({ 
  visible,
  onClose, 
  onSave, 
  editData 
  ...
  useEffect(() => {
    if (visible) {
      // 组件打开时重置业务流程属性面板状态
      setActionSequencePropertyPanelVisible(false);
  ...
  // 独立子画布窗口管理
  const [,] = useState<Map<string, { nodeId: string; position: { x: number; y: number } }>>(new Map());
  ...
  // 业务流程属性面板状态
  const [actionSequencePropertyPanelVisible, setActionSequencePropertyPanelVisible] = useState(false);
  ...
  // 关闭业务流程属性面板
  const handleCloseActionSequencePropertyPanel = useCallback(() => {
    setActionSequencePropertyPanelVisible(false);
    setSelectedBusinessProcessNode(null);
  }, []);
  ...
  // 打开独立子画布窗口 - 阶段节点已移除
  const openIndependentSubCanvas = useCallback(() => {
    return; // 阶段节点已移除
  }, []);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (clickedAddButton) {
      ...
      if (clickedNode.type === 'businessProcess') {
        ...
        setActionSequencePropertyPanelVisible(true);
        setStagePropertyPanelVisible(false);
        setSelectedStageNode(null);
      }
  ...
  }, [canvasState.isSpacePressed, getCanvasCoordinates, findAddButtonAtPosition, findNodeAtPosition, findSubCanvasAtPosition, findConnectionAtPosition, openIndependentSubCanvas]);
  ...
  return (
    ...
    >
      ...
      {/* 业务流程属性面板 - 只在画布编辑步骤显示 */}
      <ActionSequencePropertyPanel
        visible={actionSequencePropertyPanelVisible && currentStep === 1}
        businessProcessNode={selectedBusinessProcessNode}
        onSave={handleSaveBusinessProcessNode}
        onClose={handleCloseActionSequencePropertyPanel}
      />
    </Drawer>
  );
};

export default AddActionSequence;